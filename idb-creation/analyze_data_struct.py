...
def init_strings(force=False):
    ...
    for ea in idautils.Segments():
        ...
        # We only check MAIN since it takes too much time.
        if seg_name != 'MAIN':
            continue
...
def analyze_str_ptr(start_ea, end_ea):
    ...
    while ea != idc.BADADDR and ea < end_ea:
        ...
        if idc.get_str_type(str_ptr) is not None or check_string(str_ptr, 8):
            ...
            str_cnt += 1
            if str_cnt % 10000 == 0:
                print("%x: %d strings has been found. (%0.3f secs)" % (ea, str_cnt, time.time() - start_time))
...
def analyze_ida_str():
    ...
    for s in all_str:
        ...
        if any(ida_funcs.get_fchunk(ea) or (idc.get_str_type(ea) is not None) for ea in (s.ea, s.ea + s.length)):
            continue

        if check_string(s.ea, 30) and create_string(s.ea):
            str_cnt += 1
            if str_cnt % 1000 == 0:
                print("%x: %d strings has been found. (%0.3f secs)" % (s.ea, str_cnt, time.time() - start_time))
...
def fix_func_prolog(ea, end_ea=idc.BADADDR):
    ...
    while func is not None and ea < end_ea:
        ...
        if func.size() <= 8 and idc.get_func_attr(func.end_ea, idc.FUNCATTR_START) != idc.BADADDR:
            ...
            if all((func.start_ea <= xref.frm < func.end_ea) \
                    and xref.type == 21 \
                    for xref in XrefsTo(func.end_ea)):
                if func_cnt > 0 and func_cnt % 1000 == 0:
                    print ("%x <- %x: prolog merging (%d)." % (func.start_ea, func.end_ea, func_cnt))
...
def analyze_linear_sweep(start_ea, end_ea=idc.BADADDR):
    ...
    while ea < end_ea and ea != idc.BADADDR:
        ...
        cand_cnt += 1
        if cand_cnt % 10000 == 0:
            print ("%x: %d/%d function has been found (%d secs)" % (ea, func_cnt, cand_cnt, time.time() - start_time))
    ...
    print("Found %d/%d functions. (%d sec)" % (len(FUNC_BY_LS), cand_cnt, FUNC_BY_LS_TIME))
...
def analyze_func_ptr(start_ea, end_ea):
    ...
    while ea != idc.BADADDR and ea <= end_ea:
        ...
        if not is_code_segment(func_ptr):
            ea = next_addr_aligned(ea, 4)
            continue
        ...
        if func_start == idc.BADADDR:
            ...
            FUNC_BY_PTR.add(ea)
            if func_cnt % 10000 == 0:
                print("%x: %d functions has been found. (%0.3f secs)" % (ea, func_cnt, time.time() - start_time))
        ...
        if analyze_funcname(ea, func_ptr):
            ...
            if name_cnt % 10000 == 0:
                print("%x: %d names has been found. (%0.3f secs)" % (ea, name_cnt, time.time() - start_time))
            #print("%x: %x => %s" % (ea, func_ptr, func_name))

        ea = next_addr_aligned(ea, 4)

    FUNC_BY_PTR_TIME = time.time() - start_time
    print ("Found %d functions, renamed %d functions (%0.3f secs)" % (len(FUNC_BY_PTR), name_cnt, FUNC_BY_PTR_TIME))
...
def is_code_segment(ea):
    ...
    return seg_class == "CODE"


func_initialized = False
def init_functions(force=False):
    global func_initialized
    if not force and func_initialized:
        return

    # First, initialize main code prolog
    seg = ida_segment.get_segm_by_name('MAIN')
    if seg:
        ea = seg.start_ea
        while ea < seg.start_ea + 0x30:
            if ida_bytes.get_dword(ea) & 0xffff0000 == 0xe59f0000:
    ...
    for ea in idautils.Segments():
        ...
        print("Linear sweep analysis: %x -> %x (%s)" % (seg.start_ea, seg.end_ea, seg_name))
    ...
    for ea in idautils.Segments():
        ...
        print("Function pointer analysis: %x -> %x (%s)" % (seg.start_ea, seg.end_ea, seg_name))
...
#        seg = ida_segment.getseg(ea)
#        seg_name = ida_segment.get_segm_name(seg)
#        print("Analyzing %x -> %x (%s)" % (seg.start_ea, seg.end_ea, seg_name))


# below function is deprecated. use for data pointer analysis right next to a
# function
#def analyze_func_ptrs():
#    # assume there might be multiple func ptrs nearby
#    start_addr = 0x40010000
#    end_addr = 0x417F8E84
#
#    ea = start_addr
#
...
#        if ea % 0x10000 == 0:
#            print("%x: %d funcs found." % (ea, func_cnt))
#
#        func_addr = idc.get_func_attr(ea, idc.FUNCATTR_START)
#        # if function, we skip
#        if func_addr != idc.BADADDR:
#            end_addr = idc.find_func_end(ea)
#            if end_addr != idcBADADDR:
#                ea = end_addr
#            else:
...
#                else:
#                    #try other mode (thumb mode)
#                    split_sreg_range(ea, "T", get_sreg(ea,"T")^1, idc.SR_user)
#                    if idc.add_func(ea):
#                        func_cnt += 1
#                        ea = idc.find_func_end(ea)
#                    else:
#                        # still it is not a function :/
#                        split_sreg_range(ea, "T", get_sreg(ea,"T")^1, idc.SR_user)
#                        ea = ida_bytes.next_addr(ea)
#
#    print ("Created %d new functions" % (func_cnt))
#
