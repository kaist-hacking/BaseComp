...
def run_scatterload(debug=False):
    ...
    while is_changed:
        ...
        scatter_funcs = find_scatter_funcs_from_table(tables)
...
def init_segments():
    # from sub_400005C4 in BOOT of S10
    # http://infocenter.arm.com/help/index.jsp?topic=/com.arm.doc.dui0056d/BABGEHGF.html
    # DK named below_function this function
    addrs = create_func_by_prefix('__arm_set_region', '12 0F 06 EE 11 1F 06 EE 51 2F 06 EE', force=True)
    for func_addr in addrs:
        for xref in XrefsTo(func_addr):
            args = find_args(xref.frm, 4)
            no, ea, size, flag = args
            print("%x: __arm_set_region(%d, %x, %x, %x)" % (xref.frm, no, ea, size, flag))

            # calculate size and flag
            size = 1 << (((size&0x3e) >> 1)+1)
            if (flag & 0x1000) == 0:
                seg_class = "CODE"
            else:
                seg_class = "DATA"

            add_segment(ea, size, seg_class)
...
def create_func_by_prefix(func_name, prefix, force=False):
    ...
    while func_addr != idc.BADADDR:
        func_addr = ida_search.find_binary(start_addr, idc.BADADDR, prefix, 16, idc.SEARCH_DOWN)
...
def find_scatter_funcs_from_table(tables):
    scatter_func_bytes = {
        '__scatterload': [
            '2C 00 8F E2 00 0C 90 E8 00 A0 8A E0 00 B0 8B E0', # For 5G
            '0A A0 90 E8 00 0C 82 44',
    ...
    for start, end in tables.items():
        for addr in range(start, end, 16):
            src = ida_bytes.get_dword(addr)
            dst = ida_bytes.get_dword(addr+4)
            size = ida_bytes.get_dword(addr+8)
            how = ida_bytes.get_dword(addr+12)
            if how not in funcs:
                func_addr = idc.get_func_attr(how, idc.FUNCATTR_START)
                if func_addr == idc.BADADDR:
                    idc.add_func(how)
                    func_addr = how
                ...
                for name, prefixes in scatter_func_bytes.items():
                    for prefix in prefixes:
                        prefix_addr = ida_search.find_binary(func_addr, func_end, prefix, 16, idc.SEARCH_DOWN)
                        if prefix_addr != idc.BADADDR:
                            print("%s: 0x%x" % (name, func_addr))
                            funcs[func_addr] = name
                            break
                    if func_addr in funcs:
                        break
    return funcs
...
def memcpy(src, dst, length):
    ...
    # init after copy
    #ida_bytes.put_bytes(src, b'\x00'*length)
    memclr(src, length)
