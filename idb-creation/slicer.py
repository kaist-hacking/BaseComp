...
#                        # TODO: check register initialize routine.
#                        self.regs[reg] = 0
#
#                    self.regs['SP'] = (self.regs['SP'] - 4) & 0xffffffff
#                    self.memory[self.regs['SP']] = self.regs[reg]
#                    print('0x{:x}: {}, mem[0x{:x}] = 0x{:x}'.format(ea, idc.GetDisasm(ea), self.regs['SP'], self.regs[reg]))
#
#            elif mnem.startswith('POP'):
#                for reg in regs:
#                    if self.regs['SP'] not in self.memory:
#                        return
#                    print('0x{:x}: {}, {:s} = 0x{:x}'.format(ea, idc.GetDisasm(ea), reg, self.memory[self.regs['SP']]))
#                    self.regs[reg] = self.memory[self.regs['SP']]
#                    del self.memory[self.regs['SP']]
#                    self.regs['SP'] = (self.regs['SP'] + 4) & 0xffffffff

            return True

        elif any(mnem.startswith(word) for word in ['MOV', 'LDR', 'ADR', 'STR', 'ADD', 'SUB', 'MUL']):
            ...
            if mnem.startswith('MOV'):
                ...
                #print('0x{:x}: {}, mem[0x{:x}] = 0x{:x}'.format(ea, idc.GetDisasm(ea), value, self.regs[get_reg(op1)]))
                ...
                #print('0x{:x}: {}, {:s} = 0x{:x}'.format(ea, idc.GetDisasm(ea), get_reg(op1), value))
                self.regs[get_reg(op1)] = value
            ...
            #print("%x: unknown instruction: %s" % (ea, idc.GetDisasm(ea)))
            #raise NotImplemented
            return True
...
class SimpleBackwardSlicer(object):
    ...
    def find_reg_value(self, ea, reg_name, end_ea=idc.BADADDR, inter=False, end_cnt=100):
        # not in code segment
        func_start = idc.get_func_attr(ea, idc.FUNCATTR_START)
        if func_start == idc.BADADDR:
            return
    ...
    def find_reg_value_helper(self, ea, reg_name, end_ea, end_cnt, offset=None):
        # For debugging
        #print(hex(ea), ": ", idc.GetDisasm(ea), "\t<===", reg_name, offset)
        ...
        # TODO: this should be handled for inter-procedural analysis
        ...
        if any(mnem.startswith(word) for word in ['MOV', 'LDR']):
            ...
            if op2.type == ida_ua.o_reg:
                if get_reg(op2) == 'SP':
                    offset = 0
                return self.proceed_backward(ea, get_reg(op2), end_ea, end_cnt-1, offset)

            # in the stack. need to check when the value is stored
            # o_displ = [Base Reg + Index Reg + Displacement]
            elif op2.type == ida_ua.o_displ:
#                print(hex(ea), reg_name, get_reg(op1), get_reg(op2))
#                assert get_reg(op2) == 'SP'

                values = self.proceed_backward(ea, get_reg(op2), end_ea, end_cnt-1, op2.addr)
                values = set(filter(lambda x: x, values))
                if mnem.startswith('LDR'):
#                    for value in values:
#                        print(hex(ea), idc.GetDisasm(ea), value)
            ...
            # reference the memory and get the value written in the memory
                ...
                #if op2.addr in self.memory:
                #    return self.memory[op2.addr]

                # we assume that this memory is not initialized.
                if mnem.startswith('LDR'):
                    #print('===>', hex(op2.addr), hex(ida_bytes.get_dword(op2.addr)), op2)
            ...
            # immediate value, we get the value right away
                ...
                phrase_val = self.proceed_backward(ea, get_reg(op3), end_ea, end_cnt-1, offset)
                if not phrase_val:
                    return

                op2_val = self.proceed_backward(ea, get_reg(op2), end_ea, end_cnt-1, offset)
                if not op2_val:
                    return
            ...
            #print("%x: unknown operand type: %d, %s" % (ea, op2.type, idc.GetDisasm(ea)))
            #raise NotImplemented
            return
        ...
        # only checks stored stacks
            ...
            #print(hex(ea), idc.GetDisasm(ea), reg_name, get_reg(target_op))
            ...
            # reference the memory and get the value written in the memory
                ...
                #print("%x: unknown operand type: %d, %s" % (ea, target_op.type, idc.GetDisasm(ea)))
                #raise NotImplemented
                return

            if target_op == op2:
                if target_op.addr == offset: 
                    self.stack[target_op.addr] = self.proceed_backward(ea, get_reg(op1), end_ea, end_cnt-1)
                ...
                if target_op.addr == offset:
                    ...
                    self.stack[target_op.addr+4] = self.proceed_backward(ea, get_reg(op2), end_ea, end_cnt-1)
                    return self.stack[target_op.addr+4]

            # For Debug
            #print(reg_name, offset, get_reg(target_op), target_op.addr)
            ...
            if not op3 or op3.type == ida_ua.o_void:
                if op2.type == ida_ua.o_reg:
                    op1_val = self.proceed_backward(ea, reg_name, end_ea, end_cnt-1, offset)
                    if not op1_val:
                        return

                    op2_val = self.proceed_backward(ea, get_reg(op2), end_ea, end_cnt-1)
                    if not op2_val:
                        return
            ...
            if op3.type == ida_ua.o_reg:
                ...
                op3_val = self.proceed_backward(ea, get_reg(op3), end_ea, end_cnt-1, offset)
                # if we cannot fetch the value, stop the analysis
                if not op3_val:
                    return 
                ...
                if op4 and op4.type == ida_ua.o_reg:
                    op4_val = self.proceed_backward(ea, get_reg(op4), end_ea, end_cnt-1, offset)
                    if not op4_val:
                        return

                    return merge_op_vals(merge_op_vals(op2_val, op3_val, operator), op4_val, operator)
            ...
            # ADD R0, R1, #123
                ...
                return self.proceed_backward(ea, get_reg(op2), end_ea, end_cnt-1, operator(0, op3.value))
            ...
            # o_idaspec0~5
                ...
                #print("%x: unknown operand type: %d, %s" % (ea, op3.type, idc.GetDisasm(ea)))
                #raise NotImplemented
                return

#        # Search until the previous call
#        elif any(mnem.startswith(word) for word in ['B']):
#            return

        else:
#            if not mnem.startswith('BL'):
#                print("%x: unknown operand type: %d, %s" % (ea, op2.type, idc.GetDisasm(ea)))
            return self.proceed_backward(ea, reg_name, end_ea, end_cnt-1, offset)
    ...
    def proceed_backward(self, ea, reg_name, end_ea, end_cnt, offset=None):
        ...
        while xref and xref != idc.BADADDR:
            #print("%x => %x: " % (xref, ea))
            tmp_values = self.find_reg_value_helper(xref, reg_name, end_ea, end_cnt, offset)
            if tmp_values:
...
def find_mnem(target_ea, target_mnem, backward=False, threshold=0x100):
    ...
    while True:
        ...
        if backward:
            ...
            if next_ea < target_ea - threshold:
                break
